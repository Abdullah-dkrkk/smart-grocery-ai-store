...
import { Truck, Search, AlertCircle, ChevronLeft, ChevronRight } from "lucide-react"
...
export function OrdersReceived() {
  ...
  if (loading) return (
    <div className="space-y-6">
      <div className="space-y-2"><h2 className="text-2xl font-semibold">Orders Received</h2><p className="text-base text-muted-foreground">Loading orders...</p></div>
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-base text-muted-foreground">View and manage customer orders.</p>
      </div>
      ...
      {filtered.length === 0 ? (
        <Card><CardContent className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground mb-6">{search || statusFilter !== "All" ? "Try adjusting your search." : "No orders received yet."}</p>
        </CardContent></Card>
      ) : (
        <Card className="pb-0">
