...
import { Package, Search, AlertCircle, Plus, Pencil, Trash2 } from "lucide-react"
...
export function VendorProducts() {
  ...
  if (loading) return (
    <div className="space-y-6">
      <div className="space-y-2"><h2 className="text-2xl font-semibold">My Products</h2><p className="text-base text-muted-foreground">Loading your products...</p></div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="space-y-2">
          ...
          <p className="text-base text-muted-foreground">Manage your product catalog. ({filtered.length} products)</p>
        </div>
      ...
      {filtered.length === 0 ? (
        <Card><CardContent className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground mb-6">{search ? "Try a different search." : "Start adding your products."}</p>
        </CardContent></Card>
      ) : (
              ...
              <CardContent className="flex items-center gap-4 pt-6">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {product.stock_quantity <= (product.min_stock_threshold || 5) && (
                      <Badge variant="destructive" className="text-[10px]">Low Stock</Badge>
                    )}
                  </div>
                  <p className="text-sm text-muted-foreground">SKU: {product.sku || "N/A"} | Stock: {product.stock_quantity} | Price: ${Number(product.price).toFixed(2)}</p>
                </div>
                <div className="flex gap-1 shrink-0">
                  <button className="p-2 hover:bg-muted rounded-lg transition-colors cursor-pointer"><Pencil className="h-4 w-4 text-muted-foreground" /></button>
